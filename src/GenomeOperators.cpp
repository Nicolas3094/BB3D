...
Chromosome generatePermutation(uint n)
{
    std::vector<uint> v(n);
...
Chromosome codeChromosome(LoadedBins idItemsToPacked)
{
    ...
    for (uint i = 0; i < idItemsToPacked.size(); i++)
    {
...
std::vector<Chromosome> buildHeuristicChromosomes(LoadedBins allItemBins, bool reverse = false)
{
...
std::vector<Chromosome> buildCompleteHeuristicChromosomes(const LoadedBins &allBins, const uint numberOfChromosomes)
{
...
std::vector<Chromosome> buildChromosomes(const LoadedBins &allBins, const uint numberOfChromosomes)
{
    std::vector<Chromosome> chromosomePoblation;
    uint longOfChromosome = allBins.size();
...
Chromosome generateRandomRepeatedAlalleleChromosome(uint NumberCount, uint minimum, uint maximum)
{
